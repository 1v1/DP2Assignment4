...
public class FDSControlServer {
	...
	{
		System.out.println("Publishing http://localhost:7070/fdscontrol");
		Endpoint endpoint = Endpoint.create(new FDSControlImpl());
		endpoint.setExecutor(Executors.newFixedThreadPool(NUMBER_OF_THREADS));
		endpoint.publish("http://localhost:7070/fdscontrol");

		System.out.println("Publishing http://localhost:7071/fdsinfo");
